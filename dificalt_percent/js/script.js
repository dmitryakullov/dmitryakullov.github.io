...
let currency = prompt('Введите тип вашей валюты.', 'usd'), // Во данных для расчетов
  ...
  amountOfYears = +prompt(
    'Сколько лет вы будете регулярно вкаладывать деньги? (минимум 2 года)',
    ''
  );

let incomeInResault = startCapital; //  Вписуем начальную сумму в переменную для результата

for (let i = 2; i <= amountOfYears; i++) {
  //Главный расчет сложного процента
  incomeInResault = (incomeInResault + putMoneyPerYear) * (1 + percentPerYear);
}

let rightNameOfYears;
if (amountOfYears >= 11 && amountOfYears <= 19) {
  // Конструкция для правильного отображения количествава лет.
...
alert(
  'Ваш доход за ' +
    ...
    rightNameOfYears +
    ' составит ' +
    incomeInResault.toFixed() +
    ' ' +
    currency
);

let continueHold = confirm(
  'Оставить деньги под проценты, без продолжения вкладывания денег ежегодно?'
);

if (continueHold == true) {
  // Держать ли деньги дальше?

  let amountOfYears2 = +prompt(
    'Сколько лет вы планируете продолжать держать капитал, без продолжения вкладывания денег ежегодно?',
    ''
  );

  for (let i = 1; i <= amountOfYears2; i++) {
    //Главный расчет сложного процента
  ...
  alert(
    'Ваш доход за ' +
      ...
      ' ' +
      rightNameOfYears +
      ', без продолжения регулярных вложений - составит ' +
      incomeInResault.toFixed() +
      ' ' +
      currency
  );
