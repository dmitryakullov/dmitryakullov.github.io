...
try {
  mongoose.connect(
    'mongodb+srv://dimaggio224xz:224225226227@cluster0.lbam2.mongodb.net/project?retryWrites=true&w=majority',
      ...
      (async () => {
        ...
        if (!filedata) {
          ...
          if (userCheck && userCheck.active) {
            ...
            User.findByIdAndUpdate(_id, { avatar: path }, function (err) {
              if (err) {
                ...
                const token = jwt.sign(
                  { _id, nick, email, avatar: path, active, admin },
                  config.secret
                );
...
app.post('/deletepictureadmin', (req, res) => {
  (async () => {
    ...
    if (decoded.admin) {
      User.findByIdAndUpdate(userId, { avatar: 'false' }, function (err) {
        if (err) {
          res.end(JSON.stringify({ msg: 'ERROR' }));
        } else {
...
app.post('/deletepicture', (req, res) => {
  (async () => {
    ...
    User.findByIdAndUpdate(_id, { avatar: 'false' }, function (err) {
      if (err) {
        res.end(JSON.stringify({ msg: 'ERROR' }));
      } else {
        fs.unlink('./public' + decoded.avatar, function (err) {
  ...
  (async () => {
    ...
    if (userCheck && userCheck.active) {
      if (userCheck.password === oldPass) {
        User.findByIdAndUpdate(_id, { password: NewPass }, function (err, result) {
          if (err) {
            res.end(JSON.stringify({ msg: 'ERROR' }));
          } else {
...
app.post('/user/changenickemail', function (req, res) {
  (async () => {
    ...
    if (userCheck && userCheck.active) {
      ...
      User.findByIdAndUpdate(_id, changeObj, async function (err, result) {
        if (err) {
          res.end(JSON.stringify({ msg: 'ERROR2' }));
        } else {
...
app.post('/posts&users/find', function (req, res) {
  (async () => {
    ...
    const posts = await Post.find({
      $or: [{ title: new RegExp(find, 'i') }, { text: new RegExp(find, 'i') }],
    })
      .sort({ _id: -1 })
      .limit(40);
    if (posts.length !== 0) {
  ...
  (async () => {
    ...
    if (decoded.admin === true) {
      ...
      if (!user) {
        ...
        if (user.active === true) {
          User.findByIdAndUpdate(_id, { active: false }, function (err) {
            if (err) {
              res.end(JSON.stringify({ msg: 'ERROR' }));
            } else {
          ...
          User.findByIdAndUpdate(_id, { active: true }, function (err) {
            if (err) {
              res.end(JSON.stringify({ msg: 'ERROR' }));
            } else {
    ...
    if (decoded.admin === true) {
      if (skip === 0 && nickOrEmail) {
        const user = await User.findOne({
          $or: [{ nick: nickOrEmail }, { email: nickOrEmail }],
          admin: false,
        });
        if (!user || user.admin) {
          res.end(JSON.stringify({ msg: 'NOT_FOUND' }));
...
app.post('/posts/update', function (req, res) {
  (async () => {
    ...
    if (userId === decoded._id) {
      ...
      if (userCheck && userCheck.active) {
        Post.findByIdAndUpdate(_id, { title, text }, function (err, result) {
          if (err) {
            res.end(JSON.stringify({ msg: 'ERROR1' }));
          } else if (result) {
            res.end(JSON.stringify({ msg: 'SAVE' }));
          } else res.end(JSON.stringify({ msg: 'ERROR2' }));
        });
      } else {
...
app.post('/users/new', function (req, res) {
  (async () => {
    ...
    if (check1.length !== 0) {
      ...
      const token = jwt.sign(
        { _id: newUser._id, nick, email, avatar: 'false', active: true, admin: false },
        config.secret
      );
      res.end(
        JSON.stringify({
...
app.post('/', function (req, res) {
  (async () => {
    ...
    } else res.end(JSON.stringify({ msg: "HAVEN'T_TOKEN" }));
  })();
});
